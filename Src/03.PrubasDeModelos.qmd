...
tabla_cox <- data.frame(
  coef = round(modelo_inicial_summary$coefficients[, "coef"], 5),
  exp_coef = round(modelo_inicial_summary$coefficients[, "exp(coef)"], 5),
  se_coef = round(modelo_inicial_summary$coefficients[, "se(coef)"], 5),
  z = round(modelo_inicial_summary$coefficients[, "z"], 3),
  p_value = modelo_inicial_summary$coefficients[, "Pr(>|z|)"]
)
...
grafico_surv_age <- ggsurvplot(
  ...
  legend.labs = c("Jóvenes (<50)", "Adultos (>50)"),  # Texto de la leyenda
  xlab = "Tiempo (meses)",  # Cambiar texto del eje X
  ylab = "Probabilidad de supervivencia (%)",# Cambiar texto del eje Y
  title = "Edad"
)
grafico_surv_age <- grafico_surv_age$plot + theme(plot.title = element_text(hjust = 0.5, size = 20, face = "bold"))
...
grafico_surv_B2M <- grafico_surv_B2M$plot + theme(plot.title = element_text(hjust = 0.5, size = 20, face = "bold")) 
...
grafico_surv_ALB <- grafico_surv_ALB$plot + theme(plot.title = element_text(hjust = 0.5, size = 20, face = "bold")) 
...
grafico_surv_LDH <- grafico_surv_LDH$plot + theme(plot.title = element_text(hjust = 0.5, size = 20, face = "bold")) 
...
grafico_surv_MRI <- grafico_surv_MRI$plot + theme(plot.title = element_text(hjust = 0.5, size = 20, face = "bold")) 
...
grafico_surv_CYTO <- grafico_surv_CYTO$plot + theme(plot.title = element_text(hjust = 0.5, size = 20, face = "bold"))
...
km_fit <- survfit(Surv(OS_Time, OS_Censor) ~ 1, data = Arkansas_SDF)
...
grafico_comprobacion_exponencial <- ggplot(plot_data, aes(x = time_points, y = log_S)) +
  geom_point() +
  geom_smooth(method = "lm", se = TRUE, color = "blue") +  # Ajuste lineal con IC
  labs(title = "Comprobación supuesto exponencial",
  ...
  print(grafico_comprobacion_exponencial)
```

```{r}
# Ajustar el modelo de supervivencia de Weibull
weibull_model <- survreg(Surv(OS_Time, OS_Censor) ~ 1, data = Arkansas_SDF, dist = "weibull")

# Extraer los parámetros estimados
gamma <- 1 / weibull_model$scale  # Parám. de forma
lambda <- exp(weibull_model$coef) # Parám. de escala
...
grafico_comprobacion_weibull <- ggplot(plot_data, aes(x = log_time, y = log_neg_log_S)) +
  ...
  labs(title = "Comprobación supuesto Weibull",
    subtitle = "Gráfico Log(-Log S(t)) vs. Log t",
       x = "Log t",
       y = "Log(-Log S(t))") +
  ylim(-10, 0) + # Aquí está la corrección
  theme_minimal()

print(grafico_comprobacion_weibull)
```
...
ggsave(filename = "/Users/marc/Downloads/TFG/R/TFGMarcArrom/Output/Graphs_tables/Survival_graphs/grafico_comprobacion_exp.pdf", 
       plot = grafico_comprobacion_exponencial, 
       width = 10, height = 7, device = "pdf")
...
ggsave(filename = "/Users/marc/Downloads/TFG/R/TFGMarcArrom/Output/Graphs_tables/Survival_graphs/grafico_comprobacion_weibull.pdf", 
       plot = grafico_comprobacion_weibull, 
       width = 10, height = 7, device = "pdf")
```
...
qqplot(qexp(ppoints(nrow(Arkansas_SDF)),1),residuos_cox_snell_modelo_no_parametrico)
...
for (variable in variables_backward) {
  plot <- ggplot(data = Arkansas_SDF, aes_string(x = variable, y = "residuos_martingale")) +
...
variables_backward = c("AGE", "B2M", "LDH","ALB", "MRI")
for (variable in variables_backward) {
  plot <- ggplot(data = Arkansas_SDF, aes_string(x = variable, y = "Martingale_Exp")) +
    geom_point(color = "#1f78b4", alpha = 0.6) +
    geom_smooth(method = "loess", color = "red") +
    labs(title = paste("Residuos de Martingale vs", variable),
         x = variable,
...
variables_backward = c("AGE", "B2M", "LDH","ALB", "MRI")
for (variable in variables_backward) {
  plot <- ggplot(data = Arkansas_SDF, aes_string(x = variable, y = "Martingale_Weibull")) +
    geom_point(color = "#1f78b4", alpha = 0.6) +
    geom_smooth(method = "loess", color = "red") +
    labs(title = paste("Residuos de Martingale vs", variable),
         x = variable,
...
ggplot(data = Arkansas_SDF, aes(x = residuos_deviance_modelo_no_parametrico)) +
...
ggplot(data = Arkansas_SDF, aes(y = residuos_deviance_modelo_no_parametrico)) +
...
variables_backward = c("AGE", "B2M", "LDH","ALB", "MRI")
for (variable in variables_backward) {
plot_modelo_no_parametrico <- ggplot(data = Arkansas_SDF, aes_string(x = variable, y = "residuos_deviance_modelo_no_parametrico")) +
  geom_point() +
...
plot_modelo_exp <- ggplot(data = Arkansas_SDF, aes_string(x = variable, y = "residuos_deviance_modelo_exp")) +
  geom_point() +
  geom_smooth(method = "loess", color = "red", se = TRUE) +
  labs(
    title = paste("Deviance vs ",variable),
...
plot_modelo_weibull<- ggplot(data = Arkansas_SDF, aes_string(x = variable, y = "residuos_deviance_modelo_weibull")) +
  geom_point() +
  geom_smooth(method = "loess", color = "red", se = TRUE) +
  labs(
    title = paste("Deviance vs ",variable),
...
surv_obj <- Surv(Arkansas_SDF$OS_Time, Arkansas_SDF$OS_Censor)

# Calcular el C-Index para el modelo de Cox
concordance_cox <- concordance(modelo_backward, newdata = Arkansas_SDF)
c_index_cox <- concordance_cox$concordance

# Calcular el C-Index para el modelo Exponencial
concordance_exp <- concordance(modelo_exp_backward, newdata = Arkansas_SDF)
c_index_exp <- concordance_exp$concordance

# Calcular el C-Index para el modelo Weibull
concordance_weibull <- concordance(modelo_weibull_backward, newdata = Arkansas_SDF)
